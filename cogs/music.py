...
class Player(wavelink.Player):
    ...
    async def add_tracks(self, ctx, tracks):
        ...
        if isinstance(tracks, wavelink.TrackPlaylist):
            ...
            await ctx.send(f"Thêm {tracks[0].title} vô queue")
        else:
            if (track := await self.choose_track(ctx, tracks)) is not None:
                self.queue.add(track)
                await ctx.send(f"Thêm {track.title} vô queue")
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def connect_command_error(self, ctx, exc):
        if isinstance(exc, AlreadyConnectedToChannel):
            await ctx.send("Đã vô 1 voice channel rồi")
    ...
    async def play_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("Không có bài nào trong queue")
        elif isinstance(exc, NoVoiceChannel):
            await ctx.send("chưa vô voicechannel")
    ...
    async def next_command(self, ctx):
        ...
        await player.stop()
        await ctx.send("▶️ Chơi bài tiếp theo")

    @next_command.error
    async def next_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("queue đang trống")
        elif isinstance(exc, NoMoreTracks):
            await ctx.send("Không còn bài nào trong queue")
    ...
    async def previous_command(self, ctx):
        ...
        await player.stop()
        await ctx.send("chơi lại track trước")
    ...
    async def previous_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            ...
            await ctx.send("không có track trước để phát")

    @commands.command(name="shuffle")
    async def shuffle_command(self, ctx):
        player = self.get_player(ctx)
        player.queue.shuffle()
        await ctx.send("🔀 Đã đảo lộn queue")

    @shuffle_command.error
    async def shuffle_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("Không shuffle được vì queue trống")
    ...
    async def repeat_command(self, ctx, mode: str):
        ...
        await ctx.send(f"Đã repeat {mode}.")
    ...
    async def queue_command(self, ctx, show: t.Optional[int] = 10):
        ...
        embed = discord.Embed(
            title="Queue",
            description=f"Show Được {show} tracks",
            colour=ctx.author.colour,
            timestamp=dt.datetime.utcnow()
        )
        embed.set_author(name="Kết quả")
        embed.set_footer(text=f"Yêu cầu bởi {ctx.author.display_name}", icon_url=ctx.author.avatar_url)
        embed.add_field(
            name="Hiện đang phát",
            value=getattr(player.queue.current_track, "title", "Không có track đang phát"),
            inline=False
        )
    ...
    async def lyrics_command_error(self, ctx, exc):
        if isinstance(exc, NoLyricsFound):
            await ctx.send("Không thấy lyrics.")
    ...
    async def playing_command(self, ctx):
        ...
        embed.set_author(name="Đang Phát")
        embed.set_footer(text=f"Requested bởi {ctx.author.display_name}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Tên Track", value=player.queue.current_track.title, inline=False)
        embed.add_field(name="Nghệ Sĩ", value=player.queue.current_track.author, inline=False)
    ...
    async def playing_command_error(self, ctx, exc):
        if isinstance(exc, PlayerIsAlreadyPaused):
            await ctx.send("Không có track đang chạy.")
    ...
    async def skipto_command(self, ctx, index: int):
        ...
        await player.stop()
        await ctx.send(f"Phát track ở vị trí {index}.")

    @skipto_command.error
    async def skipto_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("Không có track trong queue")
        elif isinstance(exc, NoMoreTracks):
            await ctx.send("Không có vị trí đó trong queue")
    ...
    async def restart_command(self, ctx):
        ...
        await player.seek(0)
        await ctx.send("track đã restart")

    @restart_command.error
    async def restart_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("Không còn track trong queue")
