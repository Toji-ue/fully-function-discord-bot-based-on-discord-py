...
class helpCog(commands.Cog):
    ...
    async def help(self,ctx,*,user: discord.Member = None):
        ...
        embed.add_field(name='Hololive Pic',value='`hololive` `sora` `miko` `aki` `suisei` `roboco` `fubuki` `matsuri` `aqua` `shion` `haachama` `ayame` `choco` `subaru` `AZKi` `mio` `okayu` `korone` `pekora` `rushia` `marine` `flare` `noel`  `coco`  `kanata`  `watame`  `towa`  `luna` `lamy`  `nene`  `botan`  `polka`  `aloe` ',inline=False)
